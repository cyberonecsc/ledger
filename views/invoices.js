...
export function renderInvoices(mountPoint, appInstance) {
  ...
  // Calculate initial metrics for the KPI grid
  const paidInvoices = store.invoices.filter(i => i.status === 'paid');
  const outstandingInvoices = store.invoices.filter(i => i.status === 'sent');
  ...
  mountPoint.innerHTML = `
    ...
    <div class="card-grid" style="grid-template-columns: repeat(3, 1fr); margin-bottom: 25px;">
      <div class="glass-card success">
        ...
        <div class="card-value" id="kpi-paid-bills" style="margin-top:5px; font-size:22px;">${paidInvoices.length} Bills</div>
      ...
      <div class="glass-card warning">
        ...
        <div class="card-value" id="kpi-outstanding-amt" style="margin-top:5px; font-size:22px;">₹${totalOutstanding.toFixed(2)}</div>
        <div class="card-change warning" id="kpi-outstanding-bills" style="font-size:11px; color:var(--color-warning);">${outstandingInvoices.length} bills pending collection</div>
      </div>

      <div class="glass-card info">
        <span class="card-title" style="font-size:11px;">Total Invoices Issued</span>
        <div class="card-value" id="kpi-total-bills" style="margin-top:5px; font-size:22px;">${store.invoices.length} Bills</div>
  ...
  const updateKPICards = () => {
    ...
    const paid = currentInvoices.filter(i => i.status === 'paid');
    const outstanding = currentInvoices.filter(i => i.status === 'sent');
    const outstandingAmt = outstanding.reduce((sum, i) => sum + i.total, 0);

    const paidBillsEl = document.getElementById('kpi-paid-bills');
    const outstandingAmtEl = document.getElementById('kpi-outstanding-amt');
    const outstandingBillsEl = document.getElementById('kpi-outstanding-bills');
    const totalBillsEl = document.getElementById('kpi-total-bills');

    if (paidBillsEl) paidBillsEl.innerText = `${paid.length} Bills`;
    if (outstandingAmtEl) outstandingAmtEl.innerText = `₹${outstandingAmt.toFixed(2)}`;
    if (outstandingBillsEl) outstandingBillsEl.innerText = `${outstanding.length} bills pending collection`;
    if (totalBillsEl) totalBillsEl.innerText = `${currentInvoices.length} Bills`;
  };

  const redrawTable = () => {
    const query = (document.getElementById('inv-search')?.value || '').toLowerCase();
    
    // Filter invoices by the local selected active date
    let filteredInvoices = store.invoices.filter(i => i.createdAt === localActiveDate);
    
    if (query) {
      filteredInvoices = filteredInvoices.filter(inv => {
        const client = customers.find(c => c.id === inv.customerId);
        const clientName = client ? client.name.toLowerCase() : 'walk-in';
    ...
    // Bind table status selectors dynamically
    ...
    // Bind table printable receipt buttons dynamically
  ...
  // Bind toolbar Date Picker
  const invDatePickerEl = document.getElementById('inv-date-picker');
  if (invDatePickerEl) {
    invDatePickerEl.addEventListener('change', (e) => {
  ...
  document.getElementById('btn-create-inv').addEventListener('click', () => {
    const activeDate = localActiveDate;
